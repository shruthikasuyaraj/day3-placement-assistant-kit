...
def call_tool(tools: PlacementTools, placement: PlacementDb, key: str, name: str, args: dict) -> dict:
    """Run a tool, using idempotency for side-effect tools."""
    ...
    try:
        if name in tools.SIDE_EFFECTS:
            result, _ = placement.once(key, name, effect)
            return result
        ...
        return {
            ...
            "hint": f"{name} is not available yet. Tell the user."
        }
    except Exception as e:
        return {
            "error": "tool_failed",
            "hint": f"{name} failed ({type(e).__name__}). Try another way or tell the user."
        }
...
def execute_run(claimed: Claimed, *, store: RunStore, placement: PlacementDb, tools: PlacementTools, provider,
    ...
    """Drive a run to an end. Returns 'succeeded' or 'cancelled'."""
