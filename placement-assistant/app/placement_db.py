...
class PlacementDb:
    ...
    _DRIVE_SQL = (
        "SELECT d.id, c.name AS company, d.role, d.ctc_lpa, d.deadline, d.status"
        " FROM drive d JOIN company c ON c.id = d.company_id"
    )

    def get_drive(self, drive_id: int) -> Drive | None:
        r = self.conn.execute(
            self._DRIVE_SQL + " WHERE d.id = ?",
            (drive_id,)
        ).fetchone()
    ...
    def list_open_drives(self, now: datetime) -> list[Drive]:
        rows = self.conn.execute(
            self._DRIVE_SQL +
            " WHERE d.status = 'open' AND d.deadline > ? ORDER BY d.deadline",
            (now.timestamp(),)
        ).fetchall()
    ...
    def rules_for_drive(self, drive_id: int) -> list[Rule]:
        rows = self.conn.execute(
            "SELECT * FROM eligibility_rule WHERE drive_id = ? ORDER BY id",
            (drive_id,)
        ).fetchall()
    ...
    def has_application(self, student_id: int, drive_id: int) -> bool:
        return self.conn.execute(
            "SELECT 1 FROM application WHERE student_id = ? AND drive_id = ?",
            (student_id, drive_id)
        ).fetchone() is not None

    def get_slot(self, slot_id: int) -> Slot | None:
        r = self.conn.execute(
            "SELECT * FROM interview_slot WHERE id = ?",
            (slot_id,)
        ).fetchone()
    ...
    def slot_version(self, slot_id: int) -> int | None:
        r = self.conn.execute(
            "SELECT version FROM interview_slot WHERE id = ?",
            (slot_id,)
        ).fetchone()
    ...
    def free_slots(self, drive_id: int) -> list[Slot]:
        rows = self.conn.execute(
            "SELECT * FROM interview_slot WHERE drive_id = ? AND student_id IS NULL"
            " ORDER BY starts_at",
            (drive_id,)
        ).fetchall()
    ...
    def create_application(self, student_id: int, drive_id: int) -> tuple[int, bool]:
        ...
        with self.transaction() as c:
            cur = c.execute(
                "INSERT INTO application (student_id, drive_id, created_at) "
                "VALUES (?, ?, ?) "
    ...
    def claim_slot(self, slot_id: int, student_id: int, expected_version: int) -> bool:
        ...
        with self.transaction() as c:
            cur = c.execute(
                ...
                "SET student_id = ?, version = version + 1 "
    ...
    def record_notification(self, roll_no: str, message: str, dedupe_key: str) -> tuple[int, bool]:
        ...
        with self.transaction() as c:
            cur = c.execute(
                ...
                "(roll_no, message, dedupe_key, created_at) "
                "VALUES (?, ?, ?, ?) "
            ...
            row = c.execute(
                "SELECT id FROM notification WHERE dedupe_key = ?",
                (dedupe_key,)
            ).fetchone()
    ...
    def list_applications(self, student_id: int) -> list[dict]:
        rows = self.conn.execute(
            """SELECT a.id AS application_id, a.drive_id, c.name AS company, d.role, a.status, a.created_at,
                ...
                WHERE a.student_id = ? ORDER BY a.id""",
            (student_id,)
        ).fetchall()
        ...
        return [
            {
                **dict(r),
                ...
                "interview_at": _dt(r["interview_at"]) if r["interview_at"] else None
            }
            for r in rows
        ]
    ...
    def once(self, key: str, tool_name: str, effect: Callable[[], dict]) -> tuple[dict, bool]:
        """Run an effect at most once per key."""
        with self.transaction() as c:
            row = c.execute(
            ...
            c.execute(
                ...
                "VALUES (?, ?, ?, ?)",
                (key, tool_name, json.dumps(result), self.clock())
            )
